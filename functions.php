...
define("WPWIZ_THEME_URL" , get_template_directory_uri());
...
function wpwiz_setup() {
  ...
  register_nav_menus( array( 
    'header_right' => __( 'Main Menu' ,'wpwiz' ),
  ));
...
function wpwiz_content_width()
{
  if ( ! isset( $content_width ) ) $content_width = 780;
}
add_action( 'after_setup_theme', 'wpwiz_content_width', 0 );

function wpwiz_custom_header_setup() {
  add_theme_support( 'banner-header', apply_filters( 'wpwiz_custom_header_args', array(
      'width'              => 1110,
      'height'             => 555,
      'flex-height'        => true,
...
add_action( 'after_setup_theme', 'wpwiz_custom_header_setup' );
...
//enqueue scripts
function wpwiz_scripts() {
  
  wp_enqueue_style( 'wpwiz', get_template_directory_uri() .'/style.css' );
...
add_action( 'wp_enqueue_scripts', 'wpwiz_scripts' );

function wpwiz_admin_scripts() {


  $in_footer = true;
  $htm_path = WPWIZ_THEME_URL."/js/htm.js";
  wp_register_script(
    'htm',
    $htm_path,
    array(),
  ...
  wp_enqueue_script(
    ...
    WPWIZ_THEME_URL."/blocks/recent-post-slider.js",
...
function my_block_callback($props) {

  return "<div style='height: 50px; background: red; '>".print_r($props , true)."</div>";
}
